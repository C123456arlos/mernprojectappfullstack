...
const MovieSlider = ({ category }) => {
    ...
    return (
      <div className='bg-black text-white relative px5 md:px-20'
              ...
              {content.map((item) => (
                  <Link to={`/watch/${item.id}`} className='min-w-[250px] relative group' key={item.id}>
                      <div className='rounded-lg overflow-hidden'>
                          <img src={SMALL_IMAGE_BASE_URL + item.backdrop_path} alt='movie'
                           className='transition-transform duration-300 ease-in-out group-hover:scale-125'></img>
                      </div>
