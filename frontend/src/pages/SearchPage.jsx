        ...
        <div className='bg-black min-h-screen text-white'>
            ...
            <div className='container mx-auto px-4 py-8'>
                <div className='flex justify-center gap-3 mb-4'>
                    <button className={`py-2 px-4 rounded ${activeTab==='movie' ?'bg-red-600':'bg-gray-800'} hover:bg-red-700`} onClick={()=>handleTabClick('movies')}>movies</button>
                            ...
                            <div key={result.id} className='bg-gray-800 p-4 rounded'>
                                {activeTab === 'person' ? (
                                    <div  className='flex flex-col items-center'>
                                        <img src={ORIGINAL_IMAGE_BASE_URL + result.profile_path} alt={result.name} className='max-h-96 rounded mx-auto'></img>
                                        <h2 className='mt-2 text-xl font-bold'>{result.name}</h2>
                                    </div>
                                ) : (<Link to={'/watch/' + result.id} onClick={() => {
                                        setContentType(activeTab)
                                }}>
                                        <img src={ORIGINAL_IMAGE_BASE_URL + result.poster_path} alt={result.title || result.name} className='w-full h-auto rounded'></img>
