...
const HomeScreen = () => {
  ...
  return (
    ...
    <div className='relative h-screen text-white'>
        <Navbar></Navbar>
        {imgLoading && <div className='absolute top-0 left-0 w-full h-full bg-black/70 flex items-center justify-center -z-10 shimmer'></div>}
        <img src={ORIGINAL_IMAGE_BASE_URL + trendingContent?.backdrop_path} alt='image' className='absolute top-0 left-0 w-full h-full object-cover -z-50'
        onLoad={()=>setImgLoading(false)}></img>
        <div className='absolute top-0 left-0 w-full h-full bg-black/55 -z-50' aria-hidden='true'></div>
        <div className='absolute top-0 left-0 w-full h-full flex flex-col justify-center px-8 md:px-16 lg:px-32'>
          ...
          <div className='max-w-2xl'>
            ...
            <p className='mt-2 text-lg'>{trendingContent?.release_date?.split('-')[0] || 
              trendingContent?.first_air_date.split('-')[0]}{' '} | {trendingContent?.adult ? '18+' : 'PG-13'}</p>
          ...
          <div className='flex mt-8'>
            <Link to={`/watch/${trendingContent?.id}`} className='bg-white hover:bg-white/80 text-black font-bold py-2 px-4 rounded mr-4 flex items-center'>
              <Play className='size-6 mr-2 fill-black'></Play>play
            </Link>
            <Link  to={`/watch/${trendingContent?.id}`} className='bg-gray-500/70 hover:bg-gray-500 text-white py-2 px-4 rounded flex items-center'>
              <Info className='size-6 mr-2'></Info>more info
            </Link>
          </div>
        </div>
      </div>
      <div className='flex flex-col gap-10 bg-black py-10'>
        {contentType === 'movie' ? (MOVIE_CATEGORIES.map((category) => <MovieSlider key={category} category={category}></MovieSlider>)): (
          TV_CATEGORIES.map((category)=><MovieSlider key={category} category={category}></MovieSlider>
        ))
        }
      </div>
    </>
